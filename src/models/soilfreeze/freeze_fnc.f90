...
!     end function capacityhT
      ...
      if (present(tensor)) then
        if(present(quadpnt)) then 
          ...
          if(isnan(tensor(1,1))) then
           print*, "hl ", hl(pde(wat), layer, quadpnt)
    ...
    function capacityTh(pde_loc, layer, quadpnt, x) result(val)
      ...
      if(.not. fr_rate) then
        fac = 1 !icefac(quadpnt)
        val = -val*Lf*rho_ice*fac
      else
        val = 0
      end if
      if (iceswitch(quadpnt)) then
        print*, "capacityTh", val
      end if
    ...
    function capacityTT(pde_loc, layer, quadpnt, x) result(val)
      ...
      select case (drutes_config%name)
        ...
        case("freeze")
          ...
          case ("Soil")
            val = val + Cs*rho_soil*vol_soil + Ca*rho_air*th_air+Ci*rho_ice*thice
          case ("Snow")
      ...
      if (iceswitch(quadpnt)) then
        print*, "capacityTT", val
        print*, "fact", fac
        print*, "capacityTT_clap", fac*Lf*rho_ice*Lf/temp/grav*&
        vangen_elast_fr(pde_loc, layer, x = (/hl(pde(wat), layer, quadpnt)/))
        print*, "temp", temp
      end if
    ...
    function pec_tt(pde_loc, layer, quadpnt, x) result(val)
      use typy
      use global_objs
      use pde_objs
      use debug_tools
      ...
      type(integpnt_str) :: quadpnt_loc
      real(kind=rkind) :: dx
      real(kind=rkind), dimension(3) :: convect_TT
      real(kind=rkind), dimension(3,3) :: diff_TT


      if(.not. present(quadpnt)) then
        print*, x
        print*, "Quadpnt needed"
        print *, "exited from freeze_fnc::pec_tt"
        ERROR STOP
      end if
      
      call pde(heat_proc)%pde_fnc(heat_proc)%dispersion(pde_loc, layer, quadpnt, tensor = diff_TT(1:1, 1:1))
      call convectTT(pde_loc, layer, quadpnt, flux = convect_TT(1:1))

      val = convect_TT(1)/(2*diff_TT(1,1))

module freeze_helper
  ...
  contains
    ...
    function iceswitch(quadpnt) result(sw)
      ...
      if(clap) then
        if(pde(wat)%getval(quadpnt_loc) > 0) then
          Tf = 0
        else
    ...
    function hl(pde_loc, layer, quadpnt, x) result(val)
      ...
      if(isnan(val)) then
        print*, "hl is not a number! from freeze_helper::hl"
        print*, "hl", val
        print*, "hw", hw
        print*, "temp", temp
        call wait()
      ...
      if(abs(val) > huge(T_f)) then
        print*, "hl is inf! from freeze_helper::hl"
        print*, "hl", val
        print*, "hw", hw
        print*, "temp", temp
        call wait()
